...
public class CargoSpace {
    ...
    */
    public void place(Package p) {
        int[][] coords = p.getCoords();
        if(p.getType() == PackageType.PPackage || p.getType() == PackageType.TPackage || p.getType() == PackageType.LPackage)
        {
            for(int i = 0; i < coords.length; i++)
            {
                cargoSpace[coords[i][0] + curX][coords[i][1] + curY][coords[i][2] + curZ] = p.getType(); 
            }
        }
        else
        {
            ...
            for (int j = cargoSpace[0][0].length - 1; j >= 0; j--) {
                for (int k = 0; k < cargoSpace[0].length; k++) {
                    if (cargoSpace[i][k][j] == PackageType.NoPackage)
                        ...
                        System.out.print("C ");
                     else if (cargoSpace[i][k][j] == PackageType.LPackage)
                        System.out.print("L");
                    else if (cargoSpace[i][k][j] == PackageType.PPackage)
                        System.out.print("P");
                    else if (cargoSpace[i][k][j] == PackageType.TPackage)
                        System.out.print("T");
                }
                System.out.println();
            }
