...
public class HillClimbing {
    ...
    public void initialSolution() {
        packages = new PackageType[300];
        coordinates = new int[300][3];
        rotations = new int[300][3];

        for (int i=0; i<100; i++) {
            packages[i] = PackageType.APackage;
            packages[i+100] = PackageType.BPackage;
            packages[i+200] = PackageType.CPackage;
        }
    ...
    public void theHood() {
        for (int x = 0; x<NEIGHBOURHOOD_SIZE; x++) {
            ...
            int[][] newCoordinates = new int[300][3];
            int[][] newRotations = new int[300][3];
            for (int i = 0; i < coordinates.length; i++) {
            ...
            for (int i = 0; i < PACKAGE_CHANGE; i++) {
                int n = Random.randomWithRange(0, 299);
